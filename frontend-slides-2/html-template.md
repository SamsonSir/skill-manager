...
**Do NOT use CSS `~` sibling selector for hover-based show/hide.** Use JS-based hover with 400ms delay timeout.
...
JS hover handler with 400ms grace period to prevent flicker.

## Image Pipeline (Skip If No Images)

If images were provided, process them before generating HTML.
...
Use direct file paths (not base64). Save processed images with `_processed` suffix.
