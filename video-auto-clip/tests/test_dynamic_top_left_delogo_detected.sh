...
case "$MODE" in
  ...
  --regions-jsonl)
    for path in "$@"; do
      if [[ "$path" == *frame_00001.jpg* ]]; then
        printf '{"path":"%s","text":"抖音 搜索 测试店铺","regions":[{"text":"抖音 搜索 测试店铺","x":0.03,"y":0.94,"width":0.28,"height":0.04}]}\n' "$path"
      else
...
if [[ "$DELOGO_COUNT" -ne 2 ]]; then
  echo "左上角动态去水印未接入: $DELOGO_COUNT" >&2
  cat "$FILTER_SCRIPT" >&2
  exit 1
fi

grep -q "delogo=x=7:y=10:w=231:h=125:enable='between(t,0,5)'" "$FILTER_SCRIPT" || {
  echo "左上角去水印未使用动态框位" >&2
  cat "$FILTER_SCRIPT" >&2
  exit 1
}

echo "test_dynamic_top_left_delogo_detected: PASS"
