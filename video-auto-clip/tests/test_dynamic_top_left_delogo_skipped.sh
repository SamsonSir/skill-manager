...
case "$MODE" in
  ...
  --regions-jsonl)
    for path in "$@"; do
      printf '{"path":"%s","text":"高级感通勤穿搭","regions":[{"text":"高级感通勤穿搭","x":0.04,"y":0.93,"width":0.30,"height":0.05}]}\n' "$path"
    done
    ;;
  *)
    exit 1
...
if [[ "$DELOGO_COUNT" -ne 1 ]]; then
  echo "左上角正文仍被误打去水印: $DELOGO_COUNT" >&2
  cat "$FILTER_SCRIPT" >&2
  exit 1
fi

echo "test_dynamic_top_left_delogo_skipped: PASS"
