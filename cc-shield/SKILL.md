...
name: cc-shield
description: |-
  Codex account protection — disable telemetry, clean device fingerprints, safe account switching after ban.
  ...
  "Codex is collecting data", "I got banned", "switch to a new account",
  "what does Codex know about me", "帮我关掉遥测", "我被封了", "怎么换号",
  "Codex 收集了什么数据", "帮我查一下隐私", even if they don't mention this skill by name.
---

# CC Shield — Codex Account Protection
...
Codex collects telemetry through two channels:
...
Each API request carries a **device ID** (random 64-char hex in `~/.Codex.json`), **account UUID**, and **session ID**. Anthropic can see when multiple accounts share the same device ID.
...
The `DISABLE_TELEMETRY` env var is an officially supported privacy control built into Codex's source — not a hack or exploit.
...
1. Read `~/.Codex/settings.json`. If it doesn't exist, create it with `{}`.
...
3. Tell the user: telemetry is now off, takes effect on next Codex launch.
...
Tell the user to **exit Codex completely** before starting.

### Step 1 — Delete OAuth credentials

**macOS:**
```bash
security delete-generic-password -a $USER -s "Codex-credentials" 2>/dev/null
```
...
config_path = os.path.expanduser('~/.Codex.json')
...
Tell the user to launch Codex and run `/login` with the new account.
...
python3 -c "
import json, os
d = json.load(open(os.path.expanduser('~/.Codex.json')))
...
Show the user what Codex has stored locally about them.

Run these checks and present the results:

```bash
# 1. Device ID
python3 -c "import json,os; d=json.load(open(os.path.expanduser('~/.Codex.json'))); print(f'Device ID: {d.get(\"userID\",\"N/A\")}')"

# 2. Account info
python3 -c "import json,os; d=json.load(open(os.path.expanduser('~/.Codex.json'))); a=d.get('oauthAccount',{}); print(f'Email: {a.get(\"emailAddress\",\"N/A\")}'); print(f'Account UUID: {a.get(\"accountUuid\",\"N/A\")}'); print(f'Org UUID: {a.get(\"organizationUuid\",\"N/A\")}')"

# 3. Telemetry status
python3 -c "
import json,os
try:
    d=json.load(open(os.path.expanduser('~/.Codex/settings.json')))
except: d={}
env=d.get('env',{})
...
ls -la ~/.Codex/telemetry/ 2>/dev/null && echo "WARNING: Pending telemetry files found — run 'rm -rf ~/.Codex/telemetry/' to clear" || echo "No pending telemetry"

# 5. Keychain credentials (macOS only)
security find-generic-password -a $USER -s "Codex-credentials" 2>/dev/null && echo "OAuth credentials found in Keychain" || echo "No Keychain credentials"
```
...
|------|-------|-------|-----|
| Config | `~/.Codex.json` | same | same |
| Settings | `~/.Codex/settings.json` | same | same |
| Credentials | Keychain (`Codex-credentials`) | `~/.Codex/.credentials.json` | same as Linux |
| Telemetry cache | `~/.Codex/telemetry/` | same | same |
