...
# 替代已于 2026-08-26 失效的 onetotenvip.com 中转（该域名 A 记录被删除且备案失效）。
...
def fetch_gzh_trends_redfox(keyword: str, api_key: str, start_date: str = None, debug: bool = False):
    """直连红狐官方接口（替代已失效的 onetotenvip.com 中转）。

    红狐接口返回扁平的文章列表，不含原四类榜单分组，
    因此统一归入「阅读靠前」分组，其余分组留空。
    """
    import datetime as _dt

    end_date = _dt.date.today().isoformat()
    start = start_date or (_dt.date.today() - _dt.timedelta(days=7)).isoformat()
    body = json.dumps(
        {
            "keyword": keyword,
            "startDate": start,
    ...
    try:
        ...
        raise Exception(f"HTTP {exc.code}") from exc

    if payload.get("code") != 2000:
        raise Exception(f"接口返回 code={payload.get('code')} msg={payload.get('msg')}")

    articles = (payload.get("data") or {}).get("articles", []) or []
    if debug:
        print(f"红狐接口返回 {len(articles)} 篇", file=sys.stderr)

    normalized = []
    for raw in articles:
        item = dict(raw)
        item.setdefault("photoId", raw.get("id"))
        item.setdefault("accountName", raw.get("author"))
        item.setdefault("oriUrl", raw.get("url"))
        normalized.append(item)

    return {
        "keyword": keyword,
        "low_fan_explosive": [],
        "ten_w_reading": normalized,
        "original_rank": [],
        "one_w_reading": [],
    }
...
def fetch_gzh_trends(keyword: str, debug: bool = False, max_retries: int = 3, start_date: str = None):
    """调用接口获取公众号爆款数据"""
    redfox_key = (os.environ.get("REDFOX_API_KEY") or "").strip()
    if redfox_key:
        return fetch_gzh_trends_redfox(keyword, redfox_key, start_date=start_date, debug=debug)

    print(
        "警告：未配置 REDFOX_API_KEY，回退到已失效的旧中转接口（onetotenvip.com），取数必定失败。\n"
