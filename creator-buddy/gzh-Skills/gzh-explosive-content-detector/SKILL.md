...
- **旧接口 `onetotenvip.com` 已失效。** 该域名的 A 记录已于 2026-08-26 被删除（DNS 返回 NODATA，非 NXDOMAIN），且 ICP 备案失效，阿里云在网络层对该域名一律返回 403 拦截页。任何依赖它的调用都会失败，典型报错为 `nodename nor servname provided` 或 `CONNECT tunnel failed, response 502`。
...
未配置 key 时会向 stderr 打印警告并回退到已失效的旧接口（必然失败）。

**一个上游能力差异，避免误判：** 红狐接口返回扁平文章列表，不含原「低粉高阅读 / 原创靠前 / 数据增长中」三类榜单分组。改造后所有文章统一归入「阅读靠前」分组，其余分组为空 —— 这是数据源能力差异，不是取数失败。

若只需按关键词查爆款文章，同类技能 `wechat-search`（红狐官方版，已安装）功能更完整，支持相关性/热度/时效三维评分与时间范围筛选。
