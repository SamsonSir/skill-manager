...
# 旧中转服务 onetotenvip.com：2026-08-26 起 A 记录被删除且备案失效，
# 阿里云在网络层拦截该域名，服务不可用。保留仅供排查参考。
LEGACY_API_HOST_PATH = "onetotenvip.com/skill/cozeSkill/getWxCozeSkillData"
API_HOST_PATH = LEGACY_API_HOST_PATH
SOURCE = "公众号爆款文章洞察-SkillHub"

# 上游官方接口（红狐数据）。设置环境变量 REDFOX_API_KEY 后自动启用。
REDFOX_API_URL = "https://redfox.hk/story/api/gzh/search/hotArticleNew"
REDFOX_SOURCE = "公众号爆款文章洞察-WorkBuddy"
...
def get_redfox_api_key() -> str:
    """读取红狐官方接口密钥；未配置时返回空字符串。"""
    return (os.environ.get("REDFOX_API_KEY") or "").strip()


def fetch_redfox_keyword(
    keyword: str,
    start_date: str | None,
    end_date: str,
    api_key: str,
    ...
    """直接调用红狐官方接口，替代已失效的 onetotenvip.com 中转。"""
    body = json.dumps(
        {
            "keyword": keyword,
            "startDate": start_date or end_date,
            "endDate": end_date,
            "source": REDFOX_SOURCE,
        },
        ensure_ascii=False,
    ).encode("utf-8")
    ...
    if code != 2000:
        raise RuntimeError(f"接口返回 code={code} msg={payload.get('msg')}")

    data = payload.get("data") or {}
    items: list[dict[str, Any]] = []
    for raw in data.get("articles", []) or []:
        items.append(normalize_redfox_item(raw, keyword))
    return items


def normalize_redfox_item(raw: dict[str, Any], keyword: str) -> dict[str, Any]:
    """把红狐返回结构映射为本脚本统一的 item 结构。"""
    title = str(raw.get("title") or "").strip()
    summary = str(raw.get("summary") or "").strip()
    if not title:
        title = summary[:42] + ("..." if len(summary) > 42 else "")
    url = sanitize_http_url(raw.get("url") or "")
    author = str(raw.get("author") or "").strip()
    watch = parse_count(raw.get("watchCount", 0))
    out = {
        "category": "红狐热门",
        "matchedKeyword": keyword,
        "photoId": str(raw.get("id") or ""),
        "title": title or "无标题",
        "summary": summary,
        "accountId": author,
        "accountName": author,
        "fans": "",
        "publicTime": raw.get("publicTime", ""),
        "noteLink": url,
        "interactiveCount": watch,
        "likeCount": parse_count(raw.get("likeCount", 0)),
        "commentCount": parse_count(raw.get("commentCount", 0)),
        "shareCount": parse_count(raw.get("shareCount", 0)),
        "clicksCount": raw.get("clicksCount", "0"),
        "watchCount": raw.get("watchCount", "0"),
        "relevanceScore": raw.get("relevanceScore", 0),
        "totalScore": raw.get("totalScore", 0),
    }
...
def main() -> None:
    ...
    redfox_key = get_redfox_api_key()
    if not redfox_key:
        print(
            "警告：未配置 REDFOX_API_KEY，将回退到已失效的旧中转接口"
            "（onetotenvip.com 域名已失效，取数必定失败）。\n"
            "      申请地址：https://redfox.hk/settings/api-keys\n"
            "      配置命令：export REDFOX_API_KEY=<你的apikey>",
            file=sys.stderr,
        )
    ...
    report: dict[str, Any] = {
        ...
        "source": REDFOX_SOURCE if redfox_key else SOURCE,
        "backend": "redfox" if redfox_key else "legacy-onetotenvip",
        "sectors": [],
    }
