...
- **Direction C — Kimi-driven** (fork addition): this file is loaded by Kimi Code CLI and the user asks to split work with Codex. Kimi is the driver: planning, splitting, blind-arbiter ruling, and the full-review gate all happen natively inside the Kimi session — no external Claude CLI is involved. `deep_reasoner` stays inline with the Kimi driver; `fast_worker` and `arbiter` run through `hosts.codex` config via `delegate-codex.sh --host codex`. Read `references/kimi-driven.md` and follow its five phases.

All directions end with the same Partner Session Receipt; `direction` records which flow ran.
...
- Route to the driver's inline seat (Claude in Direction B, Kimi in Direction C): architecture planning, implementation strategy, the split decision itself, UI/interaction critique, final review, difficult product tradeoffs.
- Route to Codex: scaffolding, implementation, long-context code edits, tests, build fixes, repository inspection, monitoring, summaries, batch content generation.
- Route back to the same driving session when quality judgment matters or the first implementation passes technically but still needs polish.
- Keep Kimi Work/Kimi Code Goal separate from Claude Code `/goal`; prior "Goal mode" context may refer to Kimi, not Claude. When Kimi Code is the driving host, Direction C applies and no Claude session exists at all.
